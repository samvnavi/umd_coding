...
def find_box(boxed_img,detections):
    ...
    if (len(detections.boxes) == 0):
        ...
        cv2.putText(boxed_img, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.6, (0, 255, 0), 2)
        cv2.imshow('object detection',boxed_img)
        return [x1,x2,y1,y2]
    ...
    for connection, timestamp, rawdata in reader.messages(connections = rgb_connections + depth_connections):
        ...
        if time_key in rgb_msgs and time_key in depth_msgs:
            ...
            if box != 0:
                ...
                cv2.rectangle(boxed_img, (box[0], box[2]), (box[1], box[3]), (0, 255, 0), 2)
                ...
                if (counts[0] < counts[1]):
                    target_cluster = unique_labels[0]
                else:
                    target_cluster = unique_labels[1]
                ...
                # kernel_closing_small = np.ones((10,1),np.uint8)
                # closing_small = cv2.morphologyEx(mask_hsv, cv2.MORPH_CLOSE, kernel_closing_small)
                masked_image_gray = cv2.cvtColor(opening, cv2.COLOR_BGR2GRAY)
                ...
                for c in contour:
                    ...
                    if area >= min_area:  # Only keep contours with area greater than or equal to min_area
                ...
                min_area = 500# replace with your desired minimum area
                ...
                cv2.drawContours(cropped_image, big_contour, -1, (0, 255, 0), 2)
